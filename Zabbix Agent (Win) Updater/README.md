...
1. Сгенерировать архив Агента в формате CAB
  1. Скачать из репы скрипт-генератор CAB-архива (каталог **CAB**)
  2. Скачать с сайта zabbix.com нужный билд Zabbix, из раздела "Zabbix Sources"
  3. Распаковать архив, из каталога bin скопировать\перенести в корень скрипта-генератора 2 подкаталога собранных Агентов (win32 и win64), подтвердить замену
  4. Запустить make_zcab.cmd с обязательным параметром битности (без параметра - справка)
...
*Архив содержит все файлы Агента, кроме файла конфигурации (это именно обновление).*
  ...
  5. Полученный архив разместить на сервере Zabbix, в любом удобном месте, доступном через HTTP(S) (например /var/www/html/agents/, https://zbx.domain.com/agents/)

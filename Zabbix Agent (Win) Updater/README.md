...
Windows 2008 (и R2) не принимает указание **/RI 0** (отключение интервала повтора задания), поэтому для универсальности выполнения на всех ОС 2008-2016 диапазон указывается в явном виде. Однако, поскольку первый запуск задаётся текущим временем до секунд, он в итоге не производится (протухает сразу при создании задачи).
